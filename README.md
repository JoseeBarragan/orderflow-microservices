...
- **Rate limiting**: `@nestjs/throttler`, in-memory, with a stricter per-route limit on the endpoints that trigger the saga (`POST /order`, payment confirmation) on top of a looser global default
...
- **In-memory rate limiting was a deliberate, scoped-down choice, not an oversight**: `@nestjs/throttler`'s default in-memory counter only works correctly for a single running instance — the moment `api-gateway` scales horizontally behind a load balancer, each replica keeps its own counter, and a limit can be bypassed just by having requests land on different instances. The professional fix for that is a shared counter in Redis (`@nestjs/throttler` supports pluggable storage for exactly this). Since this project runs a single gateway instance and the goal was to learn EDA rather than distributed rate limiting, the in-memory version was implemented, with the Redis-backed approach understood and documented rather than built. The same reasoning applies to per-route limits: not every endpoint deserves the same threshold — `POST /order` and payment confirmation are throttled more tightly than a plain `GET`, because they're the ones that actually kick off the saga.
